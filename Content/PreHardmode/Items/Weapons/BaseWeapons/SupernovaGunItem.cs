...
namespace Supernova.Content.PreHardmode.Items.Weapons.BaseWeapons
{
	...
	public abstract class SupernovaGunItem : ModItem
	{
		...
		{
			position = player.MountedCenter + Utils.ToRotationVector2(player.itemRotation) * 7f;
		...
		public override void UseStyle(Player player, Rectangle heldItemFrame)
		{
			...
			Vector2 itemPosition = player.MountedCenter + Utils.ToRotationVector2(itemRotation) * 7f;
		...
		public override void UseItemFrame(Player player)
		{
			...
			float rotation = Utils.ToRotation(player.Center - Main.MouseWorld) * player.gravDir + 1.5707964f;
			if (animProgress < 0.4f)
			{
				// Aply gun recoil
				float recoil = -Gun.recoil;
				rotation += recoil * (float)Math.Pow((double)((0.4f - animProgress) / 0.4f), 2.0) * (float)player.direction;
			}
