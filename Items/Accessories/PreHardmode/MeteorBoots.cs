...
namespace Supernova.Items.Accessories.PreHardmode
{
    public class MeteorBoots : ModItem
    {
        ...
        {
            DisplayName.SetDefault("MeteorBoots");
        ...
        public override void UpdateAccessory(Player player, bool hideVisual = false)
        {
            ...
            int dust = Dust.NewDust(new Vector2(player.position.X, player.position.Y + 2), player.width + 2, player.height + 2, DustID.t_Meteor, player.velocity.X * 0.2f, player.velocity.Y * 0.2f, 30, default(Color), 2.3f);
            Dust.NewDust(new Vector2(player.position.X, player.position.Y + 2), player.width, player.height, DustID.Fire, player.velocity.X * 0.2f, player.velocity.Y * 0.2f, 2, default(Color), 1.7f);
            ...
            if (mp.DashTimer == downDashPlayer.MAX_DASH_TIMER)
            {
                ...
                if ((mp.DashDir == downDashPlayer.DashUp && player.velocity.Y > -mp.DashVelocity) || (mp.DashDir == downDashPlayer.DashDown && player.velocity.Y < mp.DashVelocity))
                {
                ...
                int damage = Main.rand.Next(21, 40);
                Projectile.NewProjectile(player.position.X, player.position.Y - 0, Main.rand.NextFloat(0.01f, 1), 4, ProjectileID.Meteor3, damage, 7, Main.myPlayer, 0f, 0f);
                Projectile.NewProjectile(player.position.X, player.position.Y - 25, 0, 4, ProjectileID.Meteor1, damage, 7, Main.myPlayer, 0f, 0f);
                Projectile.NewProjectile(player.position.X, player.position.Y - 50, Main.rand.NextFloat(1, -0.01f), 4, ProjectileID.Meteor2, damage, 7, Main.myPlayer, 0f, 0f);
            }
